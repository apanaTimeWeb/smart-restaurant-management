...
The following practices are explicitly **STRICTLY FORBIDDEN** within the `manager` module to maintain Enterprise-Grade Architecture:
...
5. **No Inline Types:** Do not define large interfaces inside component files. Place them in `manager_types/`.
...
7. **No Hardcoded URLs:** Never hardcode route paths (e.g., `/login`). They must be imported from `manager_url_config.ts`.

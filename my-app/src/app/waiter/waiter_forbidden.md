# Forbidden Patterns: WAITER

The following practices are explicitly **STRICTLY FORBIDDEN** within the `waiter` module to maintain Enterprise-Grade Architecture:
...
4. **No UI Logic in JSX:** Do not write complex `useEffect` or multi-step state transformations directly inside `.tsx` files. Extract them to `useWaiter[Feature].ts` hooks.
5. **No Inline Types:** Do not define large interfaces inside component files. Place them in `waiter_types/`.
...
7. **No Hardcoded URLs:** Never hardcode route paths (e.g., `/login`). They must be imported from `waiter_url_config.ts`.

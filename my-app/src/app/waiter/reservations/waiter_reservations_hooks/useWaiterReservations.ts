...
// RESPONSIBILITY: All data and mutation logic for the WaiterReservations page.
// Reads/writes RESERVATIONS + TABLES from localStorage via useLocalStorage.
// Derives upcoming and past lists via useMemo.
// No JSX — pure logic hook consumed by waiter_reservations/page.tsx.
// DATA FLOW: localStorage → useLocalStorage → useWaiterReservations
...
import type {
  WaiterReservationsFormValues,
  UseWaiterReservationsReturn,
} from "@/app/waiter/reservations/waiter_reservations_types/WaiterReservationsTypes";
 ...
 * upcoming/past lists are memoized and recalculate only when waiter_reservations change.
 *
 * @returns upcoming, past, availableTables, isSubmitting, cancellingId, handlers
 */
export function useWaiterReservations(): UseWaiterReservationsReturn {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [cancellingId, setCancellingId] = useState<string | null>(null);

  // Rule 61: No direct localStorage — hooks only
  const [waiter_reservations, setWaiterReservations] = useLocalStorage<AppReservation[]>(
  ...
  // Deps: waiter_reservations — recompute when list changes
  const now = Date.now();
   ...
   *
   * @param values - Validated form values from WaiterReservationsFormModal
   */
  const addReservation = useCallback(
    (values: WaiterReservationsFormValues) => {
      ...
      // Write reservation
      setWaiterReservations((prev) => [...prev, newReservation]);
    ...
    [setWaiterReservations, setTables]
  );
  ...
  const cancelReservation = useCallback(
    (id: string) => {
      ...
      const target = waiter_reservations.find((r) => r.id === id);

      // Update reservation status
      setWaiterReservations((prev) =>
    ...
    [waiter_reservations, setWaiterReservations, setTables]
  );

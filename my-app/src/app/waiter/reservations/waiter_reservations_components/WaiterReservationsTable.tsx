...
// RESPONSIBILITY: Renders the waiter_reservations data table with status badges,
// inline cancel confirm dialog (pessimistic UI), and per-tab empty state.
// Pure display component — no localStorage access.
// DATA FLOW: useWaiterReservations → waiter_reservations/page.tsx → WaiterReservationsTable → UI

import React, { useState, useMemo } from "react";
import { XCircle, Loader2, CalendarX, Search, Filter } from "lucide-react";
import { formatDateTime } from "@/lib/formatters";
import { AppPagination } from "@/components/ui/AppPagination";
import type { WaiterReservationsTableProps } from "@/app/waiter/reservations/waiter_reservations_types/WaiterReservationsTypes";
...
const EMPTY_MESSAGES: Record<string, string> = {
  UPCOMING: "No upcoming waiter_reservations",
  PAST:     "No past waiter_reservations",
} as const;

// ─── Main Component ───────────────────────────────────────────────────────────

/**
 * Searchable, filterable, and paginated waiter_reservations data table.
 ...
 *
 * @param waiter_reservations - Filtered list (upcoming or past) from useWaiterReservations
 ...
 */
export function WaiterReservationsTable({
  waiter_reservations,
  tab,
  cancellingId,
  onCancel,
}: WaiterReservationsTableProps) {
  ...
  // Filter waiter_reservations by search term & status
  const filtered = useMemo(() => {
    return waiter_reservations.filter((res) => {
  ...
  }, [waiter_reservations, search, statusFilter]);
            ...
            {pageItems.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-xs text-text-secondary">
                  No waiter_reservations match your search or filter.
                </td>
              </tr>
            ) : (
              pageItems.map((res) => {

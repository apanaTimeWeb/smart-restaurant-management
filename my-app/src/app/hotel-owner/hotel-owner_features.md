...
## Overview
This module is strictly isolated to the **hotel-owner** role in the Smart Restaurant Management system. In accordance with the project's **Total Role Isolation** architecture, this folder contains all pages, components, hooks, and types specifically built for the hotel-owner. This prevents cross-role bugs and AI hallucinations by isolating role-specific UI and logic.

## Directory Structure
- hotel-owner_components/: Contains all React components used exclusively by this module. Files are micro-modularized and capped in size to ensure single-responsibility.
- hotel-owner_hooks/: Contains custom React hooks. All complex business logic, state management, and data transformations are extracted here, keeping the .tsx components purely as view layers.
- hotel-owner_types/: Centralized TypeScript interfaces and types for the module to prevent inline type declarations and ensure a strict Prop Blueprint.
...
3. **Hyper-Descriptive Naming**: Ensure new components strictly adhere to the hotel-owner_[ComponentName] naming convention. The component name must exactly match the filename and end with its exact structural type (e.g. ...Modal.tsx, ...Table.tsx).

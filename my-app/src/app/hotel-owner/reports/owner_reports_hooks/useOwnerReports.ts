...
// RESPONSIBILITY: All data-fetching and calculation logic for the OwnerReports page.
...
// No JSX â€” pure logic hook consumed by owner_reports/page.tsx.
// DATA FLOW: localStorage â†’ useLocalStorage â†’ calc functions â†’ UseOwnerReportsReturn
...
import type {
  ...
  OwnerReportsSummary,
  OwnerReportsTopItem,
  OwnerReportsDailyStat,
  UseOwnerReportsReturn,
} from "@/app/hotel-owner/reports/owner_reports_types/OwnerReportsTypes";
...
const PERIOD_LABELS: Record<OwnerReportsPeriod, string> = {
  TODAY:  "Today",
  WEEK:   "This Week",
  MONTH:  "This Month",
  ALL:    "All Time",
...
/**
 * Returns the start timestamp (midnight) for a given OwnerReportsPeriod.
 ...
 */
function getPeriodStart(period: OwnerReportsPeriod): number {
  const now = new Date();
 ...
 */
function filterByPeriod<T extends { timestamp?: number; createdAt?: number }>(records: T[], period: OwnerReportsPeriod, customDate: string): T[] {
 ...
 * @returns OwnerReportsSummary object
 */
function calcSummary(records: AppSalesRecord[], advanceRecords: AppAdvanceReservation[], period: OwnerReportsPeriod, customDate: string): OwnerReportsSummary {
 ...
 * @returns Array of OwnerReportsTopItem, max TOP_ITEMS_LIMIT entries
 */
function calcTopItems(
  ...
  const aggregated = new Map<string, OwnerReportsTopItem>();
 ...
 * @returns Array of OwnerReportsDailyStat entries
 */
function calcDailyRevenue(records: AppSalesRecord[], days: number): OwnerReportsDailyStat[] {
...
/**
 * Derives all OwnerReports page data from localStorage.
 ...
 */
export function useOwnerReports(): UseOwnerReportsReturn {
  const [period, setPeriod] = useState<OwnerReportsPeriod>("MONTH");

...
// RESPONSIBILITY: Renders the owner_reservations data table with status badges,
// inline cancel confirm dialog (pessimistic UI), and per-tab empty state.
// Pure display component — no localStorage access.
// DATA FLOW: useOwnerReservations → owner_reservations/page.tsx → OwnerReservationsTable → UI

import React, { useState, useMemo } from "react";
import { XCircle, Loader2, CalendarX, Search, Filter } from "lucide-react";
import { formatDateTime } from "@/lib/formatters";
import { AppPagination } from "@/components/ui/AppPagination";
import type { OwnerReservationsTableProps } from "@/app/hotel-owner/reservations/owner_reservations_types/OwnerReservationsTypes";
...
const EMPTY_MESSAGES: Record<string, string> = {
  UPCOMING: "No upcoming owner_reservations",
  PAST:     "No past owner_reservations",
} as const;

// ─── Main Component ───────────────────────────────────────────────────────────

/**
 * Searchable, filterable, and paginated owner_reservations data table.
 ...
 *
 * @param owner_reservations - Filtered list (upcoming or past) from useOwnerReservations
 ...
 */
export function OwnerReservationsTable({
  owner_reservations,
  tab,
  cancellingId,
  onCancel,
}: OwnerReservationsTableProps) {
  ...
  // Filter owner_reservations by search term & status
  const filtered = useMemo(() => {
    return owner_reservations.filter((res) => {
  ...
  }, [owner_reservations, search, statusFilter]);
            ...
            {pageItems.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-xs text-text-secondary">
                  No owner_reservations match your search or filter.
                </td>
              </tr>
            ) : (
              pageItems.map((res) => {

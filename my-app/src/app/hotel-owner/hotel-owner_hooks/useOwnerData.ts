...
// emergencyReset — verifies Owner PIN, clears localStorage, re-seeds defaults.
// getStorageUsage — estimates bytes used across all keys vs 5MB limit.
// No JSX — pure logic hook consumed by admin/data/page.tsx.
// DATA FLOW: localStorage → useOwnerData → OwnerDataPanel → UI

import { useState, useCallback, useMemo } from "react";
import { STORAGE_KEYS, initializeLocalStorageSeeds } from "@/lib/localStorageSeeder";
import type { UseOwnerDataReturn, OwnerStorageUsage } from "@/app/hotel-owner/hotel-owner_types/OwnerTypes";
 ...
 */
export function useOwnerData(): UseOwnerDataReturn {
  ...
  const storageUsage = useMemo((): OwnerStorageUsage => {
  ...
  /**
   * Verifies the Owner PIN, clears all localStorage data, and re-seeds defaults.
   * Returns true on success, false if PIN is wrong.
   *
   * @param pin - 4-digit Owner PIN entered by user
   */

...
// DATA FLOW: localStorage → useLocalStorage → useOwnerMenu → OwnerMenuTable
//            + OwnerMenuFormModal + OwnerRecipeEditor + OwnerComboEditor

import { useState, useCallback } from "react";
import { useLocalStorage } from "@/hooks/useLocalStorage";
import { STORAGE_KEYS } from "@/lib/localStorageSeeder";
import type { AppMenuItem, AppCombo, AppInventoryItem } from "@/types/appTypes";
import type { OwnerMenuFormValues, UseOwnerMenuReturn } from "@/app/hotel-owner/hotel-owner_types/OwnerTypes";
...
/** Maps OwnerMenuFormValues → AppMenuItem (new item). */
function buildMenuItem(values: OwnerMenuFormValues): AppMenuItem {
  return {
...
/**
 * Manages all Owner Menu CRUD operations.
 ...
 */
export function useOwnerMenu(): UseOwnerMenuReturn {
   ...
   */
  const addMenuItem = useCallback((values: OwnerMenuFormValues) => {
   ...
   */
  const updateMenuItem = useCallback((id: string, values: OwnerMenuFormValues) => {
    setIsSubmitting(true);
    setMenuItems((prev) =>
      prev.map((item) => {

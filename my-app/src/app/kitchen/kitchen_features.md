...
## Directory Structure
- kitchen_components/: Contains all React components used exclusively by this module. Files are micro-modularized and capped in size to ensure single-responsibility.
- kitchen_hooks/: Contains custom React hooks. All complex business logic, state management, and data transformations are extracted here, keeping the .tsx components purely as view layers.
- kitchen_types/: Centralized TypeScript interfaces and types for the module to prevent inline type declarations and ensure a strict Prop Blueprint.
...
3. **Hyper-Descriptive Naming**: Ensure new components strictly adhere to the kitchen_[ComponentName] naming convention. The component name must exactly match the filename and end with its exact structural type (e.g. ...Modal.tsx, ...Table.tsx).

...
// RESPONSIBILITY: All Menu CRUD logic for the Kitchen module.
...
// DATA FLOW: localStorage → useLocalStorage → useKitchenMenu → KitchenMenuTable
//            + KitchenMenuFormModal + KitchenRecipeEditor + KitchenComboEditor

import { useState, useCallback } from "react";
import { useLocalStorage } from "@/hooks/useLocalStorage";
import { STORAGE_KEYS } from "@/lib/localStorageSeeder";
import type { AppMenuItem, AppCombo, AppInventoryItem } from "@/types/appTypes";
import type { KitchenMenuFormValues, UseKitchenMenuReturn } from "@/app/kitchen/kitchen_types/KitchenTypes";
...
/** Maps KitchenMenuFormValues → AppMenuItem (new item). */
function buildMenuItem(values: KitchenMenuFormValues): AppMenuItem {
  return {
...
/**
 * Manages all Kitchen Menu CRUD operations.
 ...
 */
export function useKitchenMenu(): UseKitchenMenuReturn {
   ...
   */
  const addMenuItem = useCallback((values: KitchenMenuFormValues) => {
   ...
   */
  const updateMenuItem = useCallback((id: string, values: KitchenMenuFormValues) => {
    setIsSubmitting(true);
    setMenuItems((prev) =>
      prev.map((item) => {

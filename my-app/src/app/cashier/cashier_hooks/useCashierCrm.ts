...
// RESPONSIBILITY: All CRM customer logic for the Cashier POS module.
...
// No JSX — pure logic hook consumed by CashierCrmPanel.
// DATA FLOW: localStorage → useLocalStorage → useCashierCrm → CashierCrmPanel → UI

import { useState, useCallback, useMemo } from "react";
import { useLocalStorage } from "@/hooks/useLocalStorage";
import { STORAGE_KEYS } from "@/lib/localStorageSeeder";
import type { AppCrmCustomer } from "@/types/appTypes";
import type { CashierCrmCustomer, UseCashierCrmReturn } from "@/app/cashier/cashier_types/CashierTypes";
...
/**
 * Maps AppCrmCustomer (full app type) to CashierCrmCustomer (cashier-scoped type).
 * Keeps cashier module self-contained — no direct AppCrmCustomer in UI.
 */
function mapToCashierCustomer(c: AppCrmCustomer): CashierCrmCustomer {
  return {
    phone:         c.phone,
    name:          c.name,
...
/**
 * Manages CRM customer lookup and loyalty points for the Cashier POS.
 ...
 */
export function useCashierCrm(totalAmount: number): UseCashierCrmReturn {
  ...
  const [customer,      setCustomer]      = useState<CashierCrmCustomer | null>(null);
  ...
  const searchCustomer = useCallback(
    (phone: string) => {
      ...
      setCustomer(found ? mapToCashierCustomer(found) : null);
  ...
  const addNewCustomer = useCallback(
    (name: string, phone: string) => {
      ...
      setCustomer(mapToCashierCustomer(newEntry));
      setRedeemAmountState(0);
    },
    [setCrmCustomers]
  );

...
// No JSX â€” pure logic hook consumed by cashier_reports/page.tsx.
// DATA FLOW: localStorage â†’ useLocalStorage â†’ calc functions â†’ UseCashierReportsReturn
...
/**
 * Returns the start timestamp (midnight) for a given CashierReportsPeriod.
 ...
 */
function getPeriodStart(period: CashierReportsPeriod): number {
  const now = new Date();
 ...
 */
function filterByPeriod<T extends { timestamp?: number; createdAt?: number }>(records: T[], period: CashierReportsPeriod, customDate: string): T[] {
 ...
 * @returns CashierReportsSummary object
 */
function calcSummary(records: AppSalesRecord[], advanceRecords: AppAdvanceReservation[], period: CashierReportsPeriod, customDate: string): CashierReportsSummary {
 ...
 * @returns Array of CashierReportsTopItem, max TOP_ITEMS_LIMIT entries
 */
 ...
 * @returns Array of CashierReportsDailyStat entries
 */
function calcDailyRevenue(records: AppSalesRecord[], days: number): CashierReportsDailyStat[] {
...
/**
 * Derives all CashierReports page data from localStorage.

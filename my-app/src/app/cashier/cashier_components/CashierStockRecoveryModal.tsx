...
// DATA FLOW: app_stock_alerts -> CashierStockRecoveryModal -> Cashier Actions (In Progress / Supplied) -> Sync with Admin & Kitchen
...
export interface CashierStockRecoveryModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function CashierStockRecoveryModal({
  isOpen,
  onClose,
}: CashierStockRecoveryModalProps): React.JSX.Element | null {
  ...
  const handleUpdateStatus = (
    ...
    if (newStatus === "IN_PROGRESS") {
      ...
      dispatchNotification({
        ...
        title: `Restock In Progress: ${itemName} ðŸšš`,
      ...
      dispatchNotification({
        role: "ADMIN",
        type: "LOW_STOCK_ALERT",
        title: `Stock Supplied: ${itemName} ðŸ“¦`,
      ...
      case "IN_PROGRESS":
        return (
          <span className="flex items-center gap-1 rounded-full bg-blue-500/15 border border-blue-500/30 px-2.5 py-0.5 text-xs font-bold text-blue-400 animate-pulse">
            <Truck className="h-3 w-3" /> Restock In Progress ðŸšš
          </span>
        );
      case "DISPATCHED":
        return (
          <span className="flex items-center gap-1 rounded-full bg-purple-500/15 border border-purple-500/30 px-2.5 py-0.5 text-xs font-bold text-purple-400 animate-pulse">
            <PackageCheck className="h-3 w-3" /> Stock Supplied ðŸ“¦
          </span>
        );
      default:
        return (
          <span className="flex items-center gap-1 rounded-full bg-amber-500/15 border border-amber-500/30 px-2.5 py-0.5 text-xs font-bold text-amber-500 animate-pulse">
            <AlertTriangle className="h-3 w-3" /> Alert Sent ðŸš¨
          </span>
        );
    }
  };

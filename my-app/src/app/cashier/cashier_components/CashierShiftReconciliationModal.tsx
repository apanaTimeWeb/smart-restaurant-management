...
// DATA FLOW: cashier/page.tsx â†’ CashierShiftReconciliationModal â†’ UI

import { useState } from "react";
import { X, Lock, Calculator, CheckCircle2, AlertTriangle, Printer } from "lucide-react";
import { formatCurrency } from "@/lib/formatters";
import type { CashierShiftMetrics, CashierDenominations } from "@/app/cashier/cashier_types/CashierTypes";

interface CashierShiftReconciliationModalProps {
  isOpen: boolean;
  metrics: CashierShiftMetrics;
  onClose: () => void;
}

export function CashierShiftReconciliationModal({
  isOpen,
  metrics,
  onClose,
}: CashierShiftReconciliationModalProps) {
  const [denominations, setDenominations] = useState<CashierDenominations>({
    d500: 0,
    d200: 0,
    d100: 0,
    d50:  0,
  ...
  function updateDenomination(key: keyof CashierDenominations, count: number) {
          ...
          <div className="mt-5 flex flex-col gap-5 text-xs">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 rounded-xl border border-border/50 bg-surface p-3">
                {[
                  { key: "d500", label: "â‚¹500 Notes", mult: 500 },
                  { key: "d200", label: "â‚¹200 Notes", mult: 200 },
                  { key: "d100", label: "â‚¹100 Notes", mult: 100 },
                  { key: "d50",  label: "â‚¹50 Notes",  mult: 50  },
                  { key: "d20",  label: "â‚¹20 Notes",  mult: 20  },
                  { key: "d10",  label: "â‚¹10 Notes",  mult: 10  },
                ].map((den) => (
                  <div key={den.key} className="flex flex-col gap-1 rounded-lg border border-border/40 bg-page p-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      <input
                        type="number"
                        min="0"
                        value={denominations[den.key as keyof CashierDenominations] || ""}
                        onChange={(e) => updateDenomination(den.key as keyof CashierDenominations, parseInt(e.target.value) || 0)}
                        placeholder="0"
                      ...
                      <span className="text-[10px] font-semibold text-text-secondary shrink-0">
                        = {formatCurrency((denominations[den.key as keyof CashierDenominations] || 0) * den.mult)}
                      </span>
                    </div>
                  </div>
                ))}
            ...
            <div className="flex items-center justify-between rounded-xl border border-border bg-card p-3.5 shadow-xs">
              ...
              <div className="flex flex-col items-end gap-0.5">
                ...
                >
                  {discrepancy === 0 ? "Balanced âœ“" : formatCurrency(discrepancy)}
                </span>
              </div>
            </div>
          </div>

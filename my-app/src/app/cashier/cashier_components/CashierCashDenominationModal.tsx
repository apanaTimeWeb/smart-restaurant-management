...
// Calculates physical cash total across â‚¹500, â‚¹200, â‚¹100, â‚¹50, â‚¹20, â‚¹10 notes & coins and computes variance vs expected cash.
// DATA FLOW: Physical inputs -> CashierCashDenominationModal -> Total & Variance calculation

import React, { useState } from "react";
import { Calculator, X, DollarSign, CheckCircle2, AlertTriangle } from "lucide-react";
import { formatCurrency } from "@/lib/formatters";

export interface CashierCashDenominationModalProps {
  isOpen: boolean;
  onClose: () => void;
  expectedCash?: number;
...
export function CashierCashDenominationModal({
  isOpen,
  onClose,
  expectedCash = 5000,
  onConfirmReconciliation,
}: CashierCashDenominationModalProps): React.JSX.Element | null {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            {[500, 200, 100, 50, 20, 10].map((denom) => (
              ...
              >
                <span className="font-bold text-sm text-text-primary">â‚¹{denom} Notes</span>
                <input
                  type="number"
                  min="0"
          ...
          <div className="rounded-2xl border border-border bg-page p-4 space-y-2 mt-4">
            ...
            <div className="flex justify-between text-xs font-bold pt-2 border-t border-border/60">
              ...
              >
                {variance > 0 ? `+${formatCurrency(variance)} (Excess)` : variance < 0 ? `${formatCurrency(variance)} (Shortage)` : "Balanced (â‚¹0)"}
              </span>
            </div>
          </div>
        </div>

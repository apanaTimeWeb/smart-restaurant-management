...
// RESPONSIBILITY: All data and mutation logic for the CustomerReservations page.
// Reads/writes RESERVATIONS + TABLES from localStorage via useLocalStorage.
// Derives upcoming and past lists via useMemo.
// No JSX — pure logic hook consumed by customer_reservations/page.tsx.
// DATA FLOW: localStorage → useLocalStorage → useCustomerReservations
...
import type {
  CustomerReservationsFormValues,
  UseCustomerReservationsReturn,
} from "@/app/customer/reservations/customer_reservations_types/CustomerReservationsTypes";
 ...
 * upcoming/past lists are memoized and recalculate only when customer_reservations change.
 *
 * @returns upcoming, past, availableTables, isSubmitting, cancellingId, handlers
 */
export function useCustomerReservations(): UseCustomerReservationsReturn {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [cancellingId, setCancellingId] = useState<string | null>(null);

  // Rule 61: No direct localStorage — hooks only
  const [customer_reservations, setCustomerReservations] = useLocalStorage<AppReservation[]>(
  ...
  // Deps: customer_reservations — recompute when list changes
  const now = Date.now();
   ...
   *
   * @param values - Validated form values from CustomerReservationsFormModal
   */
  const addReservation = useCallback(
    (values: CustomerReservationsFormValues) => {
      ...
      // Write reservation
      setCustomerReservations((prev) => [...prev, newReservation]);
    ...
    [setCustomerReservations, setTables]
  );
  ...
  const cancelReservation = useCallback(
    (id: string) => {
      ...
      const target = customer_reservations.find((r) => r.id === id);
    ...
    [customer_reservations, setCustomerReservations, setTables]
  );

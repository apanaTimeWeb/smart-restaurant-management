...
// RESPONSIBILITY: Reservations page shell.
// Composes ReservationsTable + ReservationsFormModal.
// All data logic delegated to useReservations hook.
// isMounted guard prevents SSR/client hydration mismatch.
// DATA FLOW: useReservations → upcoming + past + availableTables
//            → ReservationsTable + ReservationsFormModal → UI

import { useState, useEffect } from "react";
import { CalendarPlus } from "lucide-react";
import { useReservations } from "@/app/reservations/reservations_hooks/useReservations";
import { ReservationsTable } from "@/app/reservations/reservations_components/ReservationsTable";
import { ReservationsFormModal } from "@/app/reservations/reservations_components/ReservationsFormModal";
import { AuthGuard } from "@/app/auth/auth_components/AuthGuard";
import type { ReservationsTab } from "@/app/reservations/reservations_types/ReservationsTypes";

// ─── Constants (Rule 35: No magic strings) ────────────────────────────────────

const PAGE_TITLE    = "Advance Reservations"                      as const;
const PAGE_SUBTITLE = "Manage table bookings and customer slots"  as const;

const TABS: { label: string; value: ReservationsTab }[] = [
...
export default function ReservationsPage() {
  ...
  const [activeTab,   setActiveTab]   = useState<ReservationsTab>("UPCOMING");
  ...
  const activeReservations = activeTab === "UPCOMING" ? upcoming : past;
...
function ReservationsPageHeader({
  activeTab,
  upcomingCount,
  pastCount,
  onTabChange,
  ...
  const counts: Record<ReservationsTab, number> = {

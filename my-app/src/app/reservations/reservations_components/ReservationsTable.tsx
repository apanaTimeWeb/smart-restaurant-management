...
// RESPONSIBILITY: Renders the reservations data table with status badges,
// inline cancel confirm dialog (pessimistic UI), and per-tab empty state.
// Pure display component — no localStorage access.
// DATA FLOW: useReservations → reservations/page.tsx → ReservationsTable → UI

import React, { useState, useMemo } from "react";
import { XCircle, Loader2, CalendarX, Search, Filter } from "lucide-react";
import { formatDateTime } from "@/lib/formatters";
import { AppPagination } from "@/components/ui/AppPagination";
import type { ReservationsTableProps } from "@/app/reservations/reservations_types/ReservationsTypes";
...
/**
 * Searchable, filterable, and paginated reservations data table.
 ...
 *
 * @param reservations - Filtered list (upcoming or past) from useReservations
  ...
  if (reservations.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-text-secondary">
        ...
        <p className="text-sm">{EMPTY_MESSAGES[tab] ?? "No reservations"}</p>
      </div>
    );
  }

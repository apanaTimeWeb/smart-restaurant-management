...
// RESPONSIBILITY: Cashier Billing POS page shell.
// Two-panel layout: left = BillingTableSelector, right = BillingOrderSummary + panels.
// Feature 17 panels (Discount, CRM, Payment, UPI QR) wired here.
// Manages all state via useBillingOrder + useBillingCrm hooks.
// DATA FLOW: useBillingOrder → billingTables + cartItems + taxBreakdown
//            → BillingTableSelector + BillingOrderSummary + BillingDiscountPanel
//            + BillingCrmPanel + BillingSplitPaymentPanel + BillingUpiQrModal → UI

import { useState, useEffect, useCallback, useMemo } from "react";
import { Split } from "lucide-react";
import { useLocalStorage } from "@/hooks/useLocalStorage";
import { STORAGE_KEYS } from "@/lib/localStorageSeeder";
import { useBillingOrder } from "@/app/billing/billing_hooks/useBillingOrder";
import { useBillingCheckout } from "@/app/billing/billing_hooks/useBillingCheckout";
import { BillingTableSelector } from "@/app/billing/billing_components/BillingTableSelector";
import { BillingOrderSummary } from "@/app/billing/billing_components/BillingOrderSummary";
import { BillingDiscountPanel } from "@/app/billing/billing_components/BillingDiscountPanel";
import { BillingCrmPanel } from "@/app/billing/billing_components/BillingCrmPanel";
import { BillingSplitPaymentPanel } from "@/app/billing/billing_components/BillingSplitPaymentPanel";
import { BillingExtraChargesPanel } from "@/app/billing/billing_components/BillingExtraChargesPanel";
import { BillingShiftSummaryBar } from "@/app/billing/billing_components/BillingShiftSummaryBar";
import { BillingGuestSplitModal } from "@/app/billing/billing_components/BillingGuestSplitModal";
import { BillingShiftReconciliationModal } from "@/app/billing/billing_components/BillingShiftReconciliationModal";
import { BillingUpiQrModal } from "@/app/billing/billing_components/BillingUpiQrModal";
import { BillingReceiptModal } from "@/app/billing/billing_components/BillingReceiptModal";
import { BillingCashDenominationModal } from "@/app/billing/billing_components/BillingCashDenominationModal";
import { BillingApprovalCenterModal } from "@/app/billing/billing_components/BillingApprovalCenterModal";
import { BillingManagerPinModal } from "@/app/billing/billing_components/BillingManagerPinModal";
import { BillingThermalReceiptPreviewModal } from "@/app/billing/billing_components/BillingThermalReceiptPreviewModal";
import { BillingKeyboardShortcutsModal } from "@/app/billing/billing_components/BillingKeyboardShortcutsModal";
import { BillingCashCalculatorModal } from "@/app/billing/billing_components/BillingCashCalculatorModal";
import { BillingStockRecoveryModal } from "@/app/billing/billing_components/BillingStockRecoveryModal";
...
const PAGE_TITLE    = "Cashier Billing POS" as const;
...
export default function BillingPage() {
  ...
  const { isProcessing, processCheckout, buildWhatsAppLink, buildReceiptText } = useBillingCheckout();

  // Modal & Panel States
  const [paymentMode,   setPaymentMode]   = useState<BillingPaymentMode>("SINGLE");
  const [singleMethod,  setSingleMethod]  = useState<BillingSingleMethod>("CASH");
  const [splitValues,   setSplitValues]   = useState<BillingSplitPaymentValues>({ cash: 0, upi: 0, card: 0 });
  ...
  const selectedTableInfo   = billingTables.find((bt) => bt.table.id === selectedTableId);
  ...
  const handlePaymentReady = useCallback(
    (mode: BillingPaymentMode, method: BillingSingleMethod, split: BillingSplitPaymentValues) => {
      setPaymentMode(mode);
  ...
  const shiftMetrics: BillingShiftMetrics = useMemo(() => ({
    openingFloat:   2000,
    cashCollected:  4850,
    upiCollected:   6200,
    cardCollected:  3100,
  ...
  return (
    <AuthGuard allowedRoles={["CASHIER", "ADMIN"]}>
      <div className="rounded-xl border border-primary/20 bg-white/10 backdrop-blur-lg p-6 shadow-lg flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[320px_1fr]">
          ...
          <div className="flex flex-col gap-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-text-secondary">
              Pending Tables ({billingTables.length})
            </p>
            <BillingTableSelector
          ...
          {/* Right panel — order summary + extra charges + discount + CRM + payment */}
          <div className="flex flex-col gap-4">
            ...
            {/* Cashier panels — only shown when a table is selected */}
            {selectedTableId && cartItems.length > 0 && (
              <>
                <BillingExtraChargesPanel
                  customTip={customTip}
                  packagingCharge={packagingCharge}
                  onSetTip={setCustomTip}
                  onSetPackaging={setPackagingCharge}
                />
                <BillingDiscountPanel
        ...
        {/* Guest Split Modal — z-50 */}
        <BillingGuestSplitModal
        ...
        {/* Shift Till Reconciliation Z-Report Modal — z-50 */}
        <BillingShiftReconciliationModal
        ...
        <BillingApprovalCenterModal
          isOpen={isApprovalOpen}
          onClose={() => setIsApprovalOpen(false)}
        />

        {/* Manager PIN Authorization Modal */}
        <BillingManagerPinModal
        ...
        <BillingThermalReceiptPreviewModal
        ...
        <BillingKeyboardShortcutsModal
          isOpen={isShortcutsOpen}
          onClose={() => setIsShortcutsOpen(false)}
        />

        {/* Quick Cash Change Calculator Modal */}
        <BillingCashCalculatorModal

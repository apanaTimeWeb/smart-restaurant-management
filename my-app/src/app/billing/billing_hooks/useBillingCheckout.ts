...
// RESPONSIBILITY: All checkout automation logic for the Billing POS module.
...
// DATA FLOW: BillingCheckoutPayload → processCheckout → 6 localStorage writes
//            → returns success boolean → billing/page.tsx shows BillingReceiptModal
...
/**
 * Derives the PaymentMethod union value from billing panel selections.
 * SPLIT_BILL and SPLIT_PAYMENT both map to "SPLIT" in AppSalesRecord.
 */
function resolvePaymentMethod(
  paymentMode:  BillingCheckoutPayload["paymentMode"],
  singleMethod: BillingCheckoutPayload["singleMethod"]
): PaymentMethod {
...
/**
 * Orchestrates the full checkout flow for the Billing POS.
 * processCheckout runs 6 sequential localStorage writes (pessimistic — all or nothing per step).
 * buildWhatsAppLink and buildReceiptText are exposed for BillingReceiptModal.
 *
...
export function useBillingCheckout(): UseBillingCheckoutReturn {
  ...
  const processCheckout = useCallback(
    async (payload: BillingCheckoutPayload): Promise<boolean> => {
      if (isProcessing) return false;
      if (!payload.orderId || !payload.tableNumber) {
        console.warn("[useBillingCheckout] orderId or tableNumber missing — aborting checkout");
        return false;
      }

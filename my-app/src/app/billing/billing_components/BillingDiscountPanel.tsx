...
// RESPONSIBILITY: Renders the discount application panel for the Billing POS.
// Supports PERCENT, FLAT, and NC (Non-Chargeable) discount types.
// NC type requires Admin PIN (4-digit) + reason — pessimistic UI on apply.
// Applied discount shown as a green badge with a remove button.
// DATA FLOW: BillingDiscountPanel → onApply/onClear → useBillingOrder.applyDiscount
...
import type { BillingDiscountPanelProps, BillingDiscount, BillingDiscountType } from "@/app/billing/billing_types/BillingTypes";

// ─── Constants (Rule 35: No magic strings) ────────────────────────────────────

const DISCOUNT_TYPE_OPTIONS: { value: BillingDiscountType; label: string }[] = [
...
export function BillingDiscountPanel({
  ...
  // Deps: appliedDiscount — reset only when it becomes null
  useEffect(() => {
  ...
  const handleQuickDiscountTag = (type: BillingDiscountType, val: number, reason: string) => {
    reset({ type, value: val, reason, adminPin: type === "NC" ? "1234" : "" });
  };

  // ── Applied badge — show when discount is active ───────────────────────────
  if (appliedDiscount) {
    const label =
      appliedDiscount.type === "PERCENT"
        ? `${appliedDiscount.value}% off`
        : appliedDiscount.type === "NC"
        ? `NC — ${formatCurrency(appliedDiscount.value)}`
    ...
    return (
      <div className="flex items-center justify-between gap-3 rounded-2xl border border-emerald-500/40 bg-emerald-500/10 p-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-text-secondary">— {appliedDiscount.reason}</span>
        </div>
        <button
          onClick={onClear}
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} noValidate className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-4 shadow-xs">
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        <button
          type="button"
          onClick={() => handleQuickDiscountTag("FLAT", 100, "Flat ₹100 Discount")}
          className="rounded-xl border border-border bg-page px-3 py-1 text-xs font-bold text-text-primary hover:bg-primary/10 hover:border-primary/40 transition-all"
        >
          Flat ₹100
        </button>
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor="discount-value" className="text-[12px] font-medium text-text-secondary">
          {selectedType === "PERCENT" ? "Percentage (%)" : "Amount (₹)"}

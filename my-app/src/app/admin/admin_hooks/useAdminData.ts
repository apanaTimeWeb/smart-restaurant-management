...
// RESPONSIBILITY: All data safety logic for the Admin module.
...
// emergencyReset — verifies Admin PIN, clears localStorage, re-seeds defaults.
// getStorageUsage — estimates bytes used across all keys vs 5MB limit.
// No JSX — pure logic hook consumed by admin/data/page.tsx.
// DATA FLOW: localStorage → useAdminData → AdminDataPanel → UI

import { useState, useCallback, useMemo } from "react";
import { STORAGE_KEYS, initializeLocalStorageSeeds } from "@/lib/localStorageSeeder";
import type { UseAdminDataReturn, AdminStorageUsage } from "@/app/admin/admin_types/AdminTypes";
...
export function useAdminData(): UseAdminDataReturn {
  ...
  const storageUsage = useMemo((): AdminStorageUsage => {
  ...
  /**
   * Verifies the Admin PIN, clears all localStorage data, and re-seeds defaults.
   * Returns true on success, false if PIN is wrong.
   *
   * @param pin - 4-digit Admin PIN entered by user
   */

...
// RESPONSIBILITY: All data and mutation logic for the AdminReservations page.
// Reads/writes RESERVATIONS + TABLES from localStorage via useLocalStorage.
// Derives upcoming and past lists via useMemo.
// No JSX — pure logic hook consumed by admin_reservations/page.tsx.
// DATA FLOW: localStorage → useLocalStorage → useAdminReservations
 ...
 * upcoming/past lists are memoized and recalculate only when admin_reservations change.
 *
...
export function useAdminReservations(): UseAdminReservationsReturn {
  ...
  const [admin_reservations, setAdminReservations] = useLocalStorage<AppReservation[]>(
  ...
  // Deps: admin_reservations — recompute when list changes
  const now = Date.now();
   ...
   *
   * @param values - Validated form values from AdminReservationsFormModal
   */
  const addReservation = useCallback(
    (values: AdminReservationsFormValues) => {
      ...
      // Write reservation
      setAdminReservations((prev) => [...prev, newReservation]);
    ...
    [setAdminReservations, setTables]
  );
  ...
  const cancelReservation = useCallback(
    (id: string) => {
      ...
      const target = admin_reservations.find((r) => r.id === id);
    ...
    [admin_reservations, setAdminReservations, setTables]
  );

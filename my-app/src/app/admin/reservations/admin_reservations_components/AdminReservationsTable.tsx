...
// RESPONSIBILITY: Renders the admin_reservations data table with status badges,
// inline cancel confirm dialog (pessimistic UI), and per-tab empty state.
// Pure display component — no localStorage access.
// DATA FLOW: useAdminReservations → admin_reservations/page.tsx → AdminReservationsTable → UI

import React, { useState, useMemo } from "react";
import { XCircle, Loader2, CalendarX, Search, Filter } from "lucide-react";
import { formatDateTime } from "@/lib/formatters";
import { AppPagination } from "@/components/ui/AppPagination";
import type { AdminReservationsTableProps } from "@/app/admin/reservations/admin_reservations_types/AdminReservationsTypes";
...
const EMPTY_MESSAGES: Record<string, string> = {
  UPCOMING: "No upcoming admin_reservations",
  PAST:     "No past admin_reservations",
} as const;

// ─── Main Component ───────────────────────────────────────────────────────────

/**
 * Searchable, filterable, and paginated admin_reservations data table.
 ...
 *
 * @param admin_reservations - Filtered list (upcoming or past) from useAdminReservations
...
export function AdminReservationsTable({
  ...
  // Filter admin_reservations by search term & status
  const filtered = useMemo(() => {
    return admin_reservations.filter((res) => {
  ...
  }, [admin_reservations, search, statusFilter]);
            ...
            {pageItems.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-xs text-text-secondary">
                  No admin_reservations match your search or filter.
                </td>
              </tr>
            ) : (
              pageItems.map((res) => {

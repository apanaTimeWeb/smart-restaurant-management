...
// import restore, and emergency reset with "Type RESET" + Admin PIN confirm.
// All destructive actions use pessimistic UI (disabled while processing).
// Pure display component — all logic delegated to useAdminData via props.
// DATA FLOW: useAdminData → admin/data/page.tsx → AdminDataPanel → UI
...
function EmergencyResetSection({
  ...
  function handleReset() {
    ...
    if (!success) {
      setError("Incorrect Admin PIN. Try again.");
      setPin("");
    }
  }
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-danger bg-danger-bg p-5">
      ...
      {/* Admin PIN */}
      <div className="flex flex-col gap-1">
        <label className="text-[12px] font-semibold text-text-secondary">Admin PIN (4 digits)</label>
        <input
          type="password"
          maxLength={4}
          value={pin}

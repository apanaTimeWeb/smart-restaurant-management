...
// RESPONSIBILITY: Renders the Customer Public Signup Form for table reservation & pre-ordering access.
// DATA FLOW: Customer inputs → AuthCustomerSignupForm.tsx → useAuth.ts → localStorage app_users → Customer Dashboard

import React, { useState } from "react";
import { useRouter } from "next/navigation";
import { useAuth } from "@/app/auth/auth_hooks/useAuth";
import type { AuthCustomerSignupFormProps } from "@/app/auth/auth_types/AuthTypes";
import { User, Phone, Lock, Eye, EyeOff, UserPlus, AlertCircle, CheckCircle2 } from "lucide-react";

export function AuthCustomerSignupForm({ onSuccessRedirect }: AuthCustomerSignupFormProps): React.JSX.Element {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = signupCustomer(name, phone, username, password);
    ...
    setTimeout(() => {
      router.push(onSuccessRedirect || "/customer");
    }, 600);
  };
    ...
    <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 sm:p-8 shadow-2xl shadow-black/40">
      <div className="mb-6 text-center">
        ...
        <p className="mt-1 text-xs text-text-secondary">
          Create an account to reserve tables, pre-order dishes, and earn loyalty points
        </p>
      </div>

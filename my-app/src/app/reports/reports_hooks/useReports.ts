...
// RESPONSIBILITY: All data-fetching and calculation logic for the Reports page.
...
// No JSX — pure logic hook consumed by reports/page.tsx.
// DATA FLOW: localStorage → useLocalStorage → calc functions → UseReportsReturn
//            → ReportsSummaryCards + ReportsRevenueChart + ReportsTopItemsTable → UI
...
import type {
  ...
  ReportsDailyStat,
  UseReportsReturn,
} from "@/app/reports/reports_types/ReportsTypes";
...
/**
 * Returns the start timestamp (midnight) for a given ReportsPeriod.
 ...
 */
function getPeriodStart(period: ReportsPeriod): number {
  const now = new Date();
 ...
 */
function filterByPeriod<T extends { timestamp?: number; createdAt?: number }>(records: T[], period: ReportsPeriod, customDate: string): T[] {
 ...
 * @returns ReportsSummary object
 */
function calcSummary(records: AppSalesRecord[], advanceRecords: AppAdvanceReservation[], period: ReportsPeriod, customDate: string): ReportsSummary {
 ...
 * @returns Array of ReportsTopItem, max TOP_ITEMS_LIMIT entries
 */
function calcTopItems(
  ...
  const aggregated = new Map<string, ReportsTopItem>();
 ...
 * @returns Array of ReportsDailyStat entries
 */
function calcDailyRevenue(records: AppSalesRecord[], days: number): ReportsDailyStat[] {
...
/**
 * Derives all Reports page data from localStorage.
...
export function useReports(): UseReportsReturn {
  ...
  // Deps: filteredRecords + period + customDate — recalc KPIs when filtered set changes
  const summary = useMemo(
    () => calcSummary(filteredRecords, filteredAdvance, period, customDate),
    [filteredRecords, filteredAdvance, period, customDate]
  );

  // Deps: orders + menu + filteredRecords — recalc top items when any source changes
  const topItems = useMemo(
    () => calcTopItems(orders, menu, filteredRecords),
    [orders, menu, filteredRecords]
  );

  // Deps: salesHistory — always last 30 days regardless of period selector

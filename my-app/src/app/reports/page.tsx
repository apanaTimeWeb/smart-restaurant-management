...
// RESPONSIBILITY: Reports & Analytics page shell.
// Composes ReportsSummaryCards + ReportsRevenueChart + ReportsTopItemsTable.
// All data logic delegated to useReports hook.
// isMounted guard prevents SSR/client hydration mismatch.
// DATA FLOW: useReports → summary + topItems + dailyRevenue
//            → ReportsSummaryCards + ReportsRevenueChart + ReportsTopItemsTable → UI

import { useState, useEffect } from "react";
import { Download } from "lucide-react";
import { useReports } from "@/app/reports/reports_hooks/useReports";
import { ReportsSummaryCards } from "@/app/reports/reports_components/ReportsSummaryCards";
import { ReportsRevenueChart } from "@/app/reports/reports_components/ReportsRevenueChart";
import { ReportsTopItemsTable } from "@/app/reports/reports_components/ReportsTopItemsTable";
import { AuthGuard } from "@/app/auth/auth_components/AuthGuard";
import type { ReportsPeriod } from "@/app/reports/reports_types/ReportsTypes";

// ─── Constants (Rule 35: No magic strings) ────────────────────────────────────

const PAGE_TITLE    = "Reports & Analytics"                    as const;
const PAGE_SUBTITLE = "Sales performance and revenue insights" as const;

const PERIOD_TABS: { label: string; value: ReportsPeriod }[] = [
...
export default function ReportsPage() {
  ...
  // Rule 6: All data + calculation logic in hook — page is pure shell
  const { summary, topItems, dailyRevenue, period, customDate, setPeriod, setCustomDate, exportCsv } = useReports();
  ...
  return (
    <AuthGuard allowedRoles={["ADMIN", "CASHIER", "HOTEL_OWNER"]}>
      <div className="flex flex-col gap-6">
        ...
        {/* Revenue Trend Chart — last 30 days (always full range) */}
        <ReportsRevenueChart dailyRevenue={dailyRevenue} />

        {/* Top Items Table — scoped to selected period */}
        <ReportsTopItemsTable topItems={topItems} />
      </div>
    </AuthGuard>
  );
}
